...
if(realData)
    if(~hgs.homing_done)
        presentMakoResults(guiHandles,'FAILURE',...
            'Homing not done');
	log_results(hgs,'MICS Status Check','FAIL','Homing not done');
        return
    end
else
    % skip homing check
...
%--------------------------------------------------------------------------
    ...
    function resultsCallback(varargin)
        try
            ...
            % 'All Pass'
            if(all([testParamStruct(:).results] == 1)) % all pass, pass
                ...
                if(~isProcedureCanceled)
                    try
                        if( iscell(testParamStruct(testIndex).TestFunction) )
                            [testParamStruct(testIndex).results testParamStruct(testIndex).failureMsg] = feval(...
...
%--------------------------------------------------------------------------
    function [testResult failureMsg] = MICSCheck(varargin)
        ...
        try
            ...
            if(realData)
                ...
                try
                    ...
                    log_message(hgs,'setting MICS gravity constants unsuccessful','WARNING'); %indicate warning to logs
                    presentMakoResults(guiHandles,'FAILURE',...
                    sprintf('Setting MICS gravity constants unsuccessful for %.1f',armHardwareVersion));
                    return;
                end
...
%--------------------------------------------------------------------------
    function MICSCheck_exit(varargin)
        ...
        log_message(hgs,'MICS Status Check Closed');
