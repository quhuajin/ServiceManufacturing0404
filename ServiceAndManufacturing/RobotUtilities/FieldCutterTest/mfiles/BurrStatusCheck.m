...
%             'Irrigation Check' @irrigationCheck;...
            };
    case 22 % 2.2
        testParams = {...
            'Handpiece Check',@handpieceCheck;...
    ...
    case 31 %3.1
         presentMakoResults(guiHandles,'FAILURE',...
            sprintf('Burr Status Check is not supported for %.1f',armHardwareVersion));
            ...
            for i=1:length(testParamStruct) %#ok<FXUP>
                ...
                if testParamStruct(i).results==SKIP
                    resultString{end+1} = sprintf('%s failure: %s',...
                        testParamStruct(i).TestName,testParamStruct(i).failureMsg);
%                     sprintf('%s warning: %s',...
%                         testParamStruct(i).TestName,testParamStruct(i).warningMsg); %#ok<AGROW>
            ...
            elseif(any([testParamStruct(:).results] == SKIP))
                ...
                presentMakoResults(guiHandles,'FAILURE',...
                    resultString);
                if(realData)
                    log_message(hgs,sprintf(['Burr Status Check Failure (1 or more tests skipped) ']),...
                        'ERROR');
                end
...
%--------------------------------------------------------------------------
    function testResult = handpieceCheck(varargin)
        ...
        try
                ...
                while (~continueTest)
                    ...
                    if(timeLoop > timeoutHold)
                        ...
                        if( passedTest == true )
                            ...
                            if strcmp(dripTurning,'No')
                                testResult = FAILURE;
                                testParamStruct(testIndex).failureMsg = 'Irrigation Motor Not Running';
                            end
                            return;
                        end
                    end
            ...
            if(~skipAnspach) && strcmp(dripTurning,'Yes');
                % If i got here text succeeded
                testResult = PASS;
            elseif strcmp(dripTurning,'No');
                testResult = FAILURE;
                testParamStruct(testIndex).failureMsg = 'Irrigation Motor Not Running';
            else
                % test skipped
                % generate warning
                testResult = SKIP;
                testParamStruct(testIndex).failureMsg = 'Test Skipped';
            end
...
%--------------------------------------------------------------------------
    function testResult = footpedalCheck(varargin)
        ...
        try
            ...
            if(~skipFootpedal) && strcmp(dripTurning,'Yes')
                % If i got here text succeeded
                testResult = PASS;
            elseif strcmp(dripTurning,'No')
                    testResult = FAILURE;
...
%--------------------------------------------------------------------------
% Internal function to check irrigation with Anspach
%--------------------------------------------------------------------------
    function testResult = irrigationCheck(varargin)
        try
            if(realData)
                %set gravity knee
                comm(hgs,'set_gravity_constants','KNEE');
        ...
        dripTurning=questdlg(...
            'While holding down hand switch, is drip motor turning?',...
            'Drip Test','Yes','No','Yes');
        switch(dripTurning)
            case 'Yes'
                %pass
