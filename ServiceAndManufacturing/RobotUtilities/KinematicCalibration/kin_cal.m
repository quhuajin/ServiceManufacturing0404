...
% Check if the passed argument is a hgs_robot object
if (nargin==1)
    if ~isa(tgs,'hgs_robot')
        if ~strcmp(tgs,'NO_ROBOT')
...
% Create generic Mako GUI with an extra panel
guiHandles = generateMakoGui(scriptName,[],tgs, true);
if isa(tgs,'hgs_robot')
    log_message(tgs,'Kinematic Calibration Script Started');
end
...
% Setup the main button function
nextExecutionStep = 'SCRIPT_START';
set(guiHandles.mainButtonInfo,'UserData',nextExecutionStep);
updateMainButtonInfo(guiHandles,@mainKincalFunction);

% save additional data to gui handles structure
guiHandles.tgs = tgs;
hndls = get(guiHandles.uiPanel,'children');
set(hndls, 'visible', 'off', 'BackgroundColor', ...
get(guiHandles.uiPanel,'BackgroundColor'));
drawnow;

%------------------------------------------------------------------------------
% Main Sequencing function for kincal
%--------------------------------------------------------------------------
    function mainKincalFunction(buttonHandle,varargin)
        nextExecutionStep = get(buttonHandle,'UserData');

        switch nextExecutionStep
            case 'SCRIPT_START'
                updateMainButtonInfo(guiHandles,...
                    'Click to select ballbar file');
                nextExecutionStep = 'SELECT_FILE';
            case 'SELECT_FILE'
                if (openFile)
                    updateMainButtonInfo(guiHandles,...
                        'Start Processing Selected file');
                    nextExecutionStep = 'START_PROCESSING';
                end
                    ...
                    switch guiHandles.calibData.baseBall
                        case 'BASEBALL_LEFT_CALIB'
                            log_results(tgs,guiHandles.scriptName,guiHandles.calibData.info,...
                                sprintf('Kinematic Calibration %s on Lefty Configuration',guiHandles.calibData.info),...
                                LogResults);
                        case 'BASEBALL_RIGHT_CALIB'
                            log_results(tgs,guiHandles.scriptName,guiHandles.calibData.info,...
                                sprintf('Kinematic Calibration %s on Righty Configuration',guiHandles.calibData.info),...
                                LogResults);
                    end
                end
                if ~strcmp(guiHandles.calibData.info,'FAILURE')
                
                    if  isa(guiHandles.tgs,'hgs_robot') 
                        updateMainButtonInfo(guiHandles,'pushbutton',...
                                             'Save results to robot');
                        nextExecutionStep = 'SAVE_RESULTS';
                    else
            ...
            case 'SAVE_RESULTS'
              ...
              set(guiHandles.mainButtonInfo,'FontSize',0.1);
            otherwise
                error('Script in unknown state');
        end
        set(buttonHandle,'UserData',nextExecutionStep);
    end
...
%------------------------------------------------------------------------------
    function calibData = kinCalFcn(guiHandles)
        ...
        set(guiHandles.file_name, 'String', [nam, ext]);
