function makeRobotUtilities(option)

% makeRobotUtilities Compile all the robot utilites to binaries
%
% Syntax:
%   makeRobotUtilities
...
% list of scripts to be installed in voyager
voyagerScripts = {
    'PresurgeryCheck',...
    'ArmStatusCheck',...
    'HomeRobot',...
    'CombinedAccuracyCheck',...
    'BrakeCheck',...
    'CheckAngleDiscrepancy',...
    'BurrStatusCheck',...
    'MICSStatusCheck',...
    'BallBarAccuracyCheck',...
    'AutoBallBarAccuracyCheck',...
    'CameraAccuracyCheck',...
    'ViewRobotLogs',...
    'ArmLogger',...
    'RestartArmSoftware',...
...
for i=1:length(fileList)
    ...
    if fileList(i).isdir ...
        ...
        if ~isempty(prjFile)
            ...
            waitCount = 0;
            waitLimit = 60*10; % seconds
            disp('');
            
            while ((~exist(fullfile(COMPILE_DIR,[execFile,'.exe']),'file')) ...
                    && (~exist(fullfile(COMPILE_DIR,execFile),'file')) ...
                    && (~exist(fullfile(BIN_DIR,execFile),'file')) ...
                    && (waitCount<waitLimit))
                fprintf('*');
                pause(5);
